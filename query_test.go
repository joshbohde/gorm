...
func TestStringWhereAgainstIncompleteParentheses(t *testing.T) {
	type AddressByZipCode struct {
		ZipCode string `gorm:"primary_key"`
		Address string
	}
	...
	DB.AutoMigrate(&AddressByZipCode{})
	DB.Create(&AddressByZipCode{ZipCode: "00503", Address: "Holtsville"})

	var address AddressByZipCode
	var addresses []AddressByZipCode
	_ = DB.Where("address_by_zip_codes=00503)) UNION ALL SELECT NULL,version(),current_database(),NULL,NULL,NULL,NULL,NULL--").Find(&addresses).GetErrors()
	if len(addresses) > 0 {
		t.Errorf("Fetch a record from with a string that has incomplete parentheses should be fail, zip code is %v", address.ZipCode)
	}
